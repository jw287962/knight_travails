...
function allMoves(position = node(), x = 0, y = 0, counter = 0) {
  ...
  // if(!visitedLeftDown)
  position.leftdown = node(
    [position.value[0] - 2, position.value[1] - 1],
    position.previousPosition
  );
  // if(!visitedLeftUp)
  position.leftup = node(
    [position.value[0] - 2, position.value[1] + 1],
    position.previousPosition
  );
  // if(!visitedUpleft)
  position.upleft = node(
    [position.value[0] - 1, position.value[1] + 2],
    position.previousPosition
  );
  // if(!visitedUpRight)
  position.upright = node(
    [position.value[0] + 1, position.value[1] + 2],
    position.previousPosition
  );
  // if(!visitedRightDown)
  position.rightdown = node(
    [position.value[0] + 2, position.value[1] - 1],
    position.previousPosition
  );
  // if(!visitedRightUp)
  position.rightup = node(
    [position.value[0] + 2, position.value[1] + 1],
    position.previousPosition
  );
  // if(!visitedDownLeft)
  position.downleft = node(
    [position.value[0] - 1, position.value[1] - 2],
    position.previousPosition
  );
  // if(!visitedDownRight)
  position.downright = node(
    [position.value[0] + 1, position.value[1] - 2],
    position.previousPosition
  );
